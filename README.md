...
This repository contains the source code for two web applications built with [Streamlit](https://streamlit.io/)＋[Python](https://docs.python.org/ja/3/). One is a **Blackjack** card game, and the other is a **sentiment‑analysis app** that uses models hosted on [Hugging Face](https://huggingface.co/). Both applications are deployed on the Streamlit Community Cloud, so you can try them out using the URLs below.
...
このリポジトリでは、[Streamlit](https://streamlit.io/)＋[Python](https://docs.python.org/ja/3/)ベースのWebアプリケーション2点のコードを公開しています。カードゲームの**ブラックジャック**と、[Hugging Face](https://huggingface.co/)にあるモデルを用いた**感情分析アプリ**です。これらはStreamlitコミュニティで運用されているので、以下のURLから実際に試せます。
